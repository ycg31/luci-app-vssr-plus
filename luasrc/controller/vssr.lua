...
function index()
    ...
    if nixio.fs.access("/usr/bin/ssr-redir") then
        entry({"admin", "vpn", "vssr"},alias("admin", "vpn", "vssr", "client"), _("vssr"),10).dependent = true                                  -- 首页
       ...
       entry({"admin", "vpn", "vssr", "subscription"},cbi("vssr/subscription"), _("Subscription Managenent"),13).leaf = true
        entry({"admin", "vpn", "vssr", "control"},cbi("vssr/control"),_("Access Control"),14).leaf = true                                            -- 访问控制
   entry({"admin", "vpn", "vssr", "servers-list"},cbi("vssr/servers-list"), _("Servers List"),15).leaf = true
